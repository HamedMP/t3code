...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Matrix (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Matrix (Nightly)");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "Matrix (Alpha) 1.2.3 Installer",
      ...
      // entry advertises MimeType=x-scheme-handler/matrixos; for OAuth deep links.
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Matrix", schemes: ["matrixos", "matrixos-dev"] },
      ]);
      assert.equal((linux.linux as Record<string, unknown>).executableName, "matrix");
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).desktop, {
        entry: { StartupWMClass: "matrix" },
      });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.matrixos.desktop</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.matrixos.desktop");
