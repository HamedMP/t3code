  ...
  it("removes a Clerk virtual pathname and callback params while preserving the desktop route", () => {
    expect(
      resolveClerkSignInProps(
        "matrixos://app/CLERK-ROUTER/VIRTUAL/sign-up?__clerk_status=complete#/settings/connections",
        true,
      ),
    ).toEqual({
      forceRedirectUrl: "matrixos://app/#/settings/connections",
      signUpForceRedirectUrl: "matrixos://app/#/settings/connections",
    });
  });

  it("preserves a clean development desktop route", () => {
    expect(resolveClerkSignInProps("matrixos-dev://app/#/settings/general", true)).toEqual({
      forceRedirectUrl: "matrixos-dev://app/#/settings/general",
      signUpForceRedirectUrl: "matrixos-dev://app/#/settings/general",
    });
  });
});
