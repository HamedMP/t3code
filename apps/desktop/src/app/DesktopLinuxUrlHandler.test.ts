...
describe("DesktopLinuxUrlHandler", () => {
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "t3code",
      desktopEntryPath: "/home/alice/.local/share/applications/matrixos-url-handler.desktop",
    ...
    assert.equal(
      ...
      "/home/alice/.local/share/applications/matrixos-url-handler.desktop",
    );
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/matrixos;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "matrixos-url-handler.desktop", "x-scheme-handler/matrixos"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/matrixos-url-handler.desktop",
        }),
      });
