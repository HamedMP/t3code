// This file mostly exists because we want dev mode to say "Matrix (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Matrix (Dev)" : "Matrix (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.matrixos.desktop.dev.${devBundleIdSuffix || "local"}`
  : "com.matrixos.desktop";
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Matrix name.
