...
// `matrix-server service update`. Keep runtime imports limited to Node built-ins.
...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  return {
    versionDir,
    entryPath: NodePath.join(versionDir, "node_modules", "matrix-server", "dist", "bin.mjs"),
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected matrix-server@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
async function main(): Promise<void> {
  ...
  if (baseDir === undefined || baseDir === "") {
    throw new Error("T3CODE_HOME is required by the Matrix Server service launcher.");
  }
