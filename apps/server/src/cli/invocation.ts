 ...
 * Detection is best-effort; callers must fail closed to a plain `matrix-server` command.
 */
...
/**
 * The Matrix Server package spec to suggest. The literal spec the user typed
 * (e.g. `matrix-server@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "matrix-server@nightly" : "matrix-server";
}

/**
 * Render a `matrix-server <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx matrix-server pair` suggests
 * `npx matrix-server serve`, a global install suggests `matrix-server serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `matrix-server ${input.subcommand}`;
  }
