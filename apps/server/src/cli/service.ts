...
export function formatServiceStatus(
  ...
  if (!status.supported) {
    return "Matrix Server service\n  Status: unavailable on this machine\n  Supported on: Linux with systemd";
  }
  if (!status.installed) {
    return "Matrix Server service\n  Status: not installed\n  Next: Run `matrix-server service install`.";
  }
  return [
    "Matrix Server service",
    `  Status: ${status.current ? `installed · matrix-server@${cliVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...(status.current ? [] : ["  Next: Run `npx matrix-server@latest service update`."]),
  ].join("\n");
}
...
const serviceInstallCommand = Command.make("install", projectLocationFlags).pipe(
  Command.withDescription("Install Matrix Server as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Matrix Server service is already installed with matrix-server@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Matrix Server service with matrix-server@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
const serviceUpdateCommand = Command.make("update", projectLocationFlags).pipe(
  Command.withDescription(
    "Update or repair the background service using this CLI version. Use `npx matrix-server@latest service update` for the latest release.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Matrix Server service is already using matrix-server@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the Matrix Server background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed
            ? "Removed the Matrix Server service."
            : "Matrix Server service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the Matrix Server background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("Matrix Server is already set up to run in the background on this machine.");
    return true;
  }
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed Matrix Server service needs an update or repair. Update it now?"
        : "Run Matrix Server in the background whenever this machine boots? " +
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the Matrix Server background service."),
