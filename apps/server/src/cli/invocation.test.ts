...
it("re-suggests the nightly channel only for nightly builds", () => {
  assert.equal(suggestedPackageSpec("0.0.31-nightly.20260729"), "matrix-server@nightly");
  assert.equal(suggestedPackageSpec("0.0.31"), "matrix-server");
});
...
it("formats serve suggestions to match the launching command", () => {
  assert.equal(
    ...
    "npx matrix-server@nightly serve",
  );
  assert.equal(
    formatCliCommand({
      subcommand: "serve",
