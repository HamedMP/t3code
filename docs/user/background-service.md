# Running Matrix Server in the Background

On a Linux host, Matrix Server can run as a background service for your user. It starts when the machine
boots and keeps running after you log out.

## Manage the Service

Install it with the latest Matrix Server release:
...
```sh
npx matrix-server@latest service uninstall
```

Updating restarts Matrix Server briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The systemd unit runs a small stable launcher. Exact Matrix Server versions are installed separately, so
...
Signing out of the optional relay does not remove the service. Use `matrix-server service uninstall`
when you no longer want Matrix Server to start in the background.
