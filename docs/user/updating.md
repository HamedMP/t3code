# Keeping Matrix in Sync

The Matrix web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Matrix shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                 |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Matrix Server Linux background service. Select the button and leave Matrix open while it prepares, tests, restarts, and reconnects.                      |
| **Update the desktop app** | Open the Matrix desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Matrix Server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Matrix does not update connected
servers silently in the background.

An older background-service launcher may ask you to run the exact
`npx matrix-server@<version> service update` command on the server machine. That one local update installs the
...
**Copy update command** gives you `npx matrix-server@<client-version>`, which relaunches the server directly
at the matching version. Add whatever startup options you normally use.

If the server instead runs as the Matrix Server background service, update the service on the host and
pin the same version:

```sh
npx matrix-server@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx matrix-server@latest service update`
...
See [Running Matrix Server in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx matrix-server@<client-version>`, replacing
